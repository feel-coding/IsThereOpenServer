...
public class CafeController {
    ...
    public void init() {
        ...
        cafeMap.put(7, new Cafe(7, "커피베이 성신여대점", "서울 성북구 보문로34길 80 1층", "매일 10:00 - 23:00", 4.2, CLOSE));
    }

    @GetMapping("/cafe/{cafeSeq}")
    ...
    @PutMapping("/cafe/{cafeSeq}")
    public void putCafe(@PathVariable("cafeSeq") Integer cafeSeq, @RequestParam("name") String name, @RequestParam("address") String address, @RequestParam("runtime") String runtime, @RequestParam("rate") Double rate) {
        cafeMap.put(cafeSeq, new Cafe(cafeSeq, name, address, runtime, rate, UNKNOWN));
    }

    @PostMapping("/cafe/{cafeSeq}")
