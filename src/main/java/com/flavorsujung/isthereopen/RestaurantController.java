...
public class RestaurantController {
    ...
    public void init() {
        ...
        restaurantMap.put(9, new Restaurant(9, "이름없는파스타 성신여대점", "서울 성북구 동소문로20다길 33", "매일 11:30 - 20:30", 4.0, BREAKTIME));
    }
    ...
    public List<Restaurant> getCafeList() {
        return new ArrayList<Restaurant>(restaurantMap.values());
    }

    @PutMapping("/restaurant/{restaurantSeq}")
    public void putRestaurant(@PathVariable("cafeSeq") Integer cafeSeq, @RequestParam("name") String name, @RequestParam("address") String address, @RequestParam("runtime") String runtime, @RequestParam("rate") Double rate) {
        restaurantMap.put(cafeSeq, new Restaurant(cafeSeq, name, address, runtime, rate, UNKNOWN));
    }
    ...
    public void postCurrentState(@PathVariable("restaurantSeq") Integer restaurantSeq, @RequestParam("openState") Integer openState) {
        restaurantOpenReviewMap.get(restaurantSeq).setOpenState(openState);
    }
}
